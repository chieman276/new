...
class studentModel
{
    ...
    function class10($class10)
    {
        global $connect;
        $sql = "SELECT * FROM `students` WHERE `teacherID` LIKE '%1%'";
        $stmt = $connect->query($sql);
        //thiết lập kiểu dữ liệu trả về
        $stmt->setFetchMode(PDO::FETCH_OBJ);
        $rows = $stmt->fetchAll();
    ...
    function class11($class11)
    {
        global $connect;
        $sql = "SELECT * FROM `students` WHERE `teacherID` LIKE '%2%'";
        $stmt = $connect->query($sql);
        //thiết lập kiểu dữ liệu trả về
        $stmt->setFetchMode(PDO::FETCH_OBJ);
        $rows = $stmt->fetchAll();
    ...
    function class12($class12)
    {
        global $connect;
        $sql = "SELECT students.*,teachers.name AS teacherName FROM `students` JOIN teachers ON teachers.id = students.teacherID 
         WHERE `teacherID` LIKE '%3%'";
        $stmt = $connect->query($sql);
        //thiết lập kiểu dữ liệu trả về
        $stmt->setFetchMode(PDO::FETCH_OBJ);
        $rows = $stmt->fetchAll();
